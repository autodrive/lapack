/* ssytf2_rk.f -- translated by f2c (version 20100827).
...
/* > \brief \b SSYTF2_RK computes the factorization of a real symmetric indefinite matrix using the bounded Bu
...
/*  ===================================================================== */
/* Subroutine */ int ssytf2_rk__(char *uplo, integer *n, real *a, integer *
	lda, real *e, integer *ipiv, integer *info, ftnlen uplo_len)
{
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax > 1) {
		    ...
		    stemp = (r__1 = a[itemp + imax * a_dim1], dabs(r__1));
		    if (stemp > rowmax) {
			rowmax = stemp;
			jmax = itemp;
		    }
...
/*                 ABS( A( IMAX, IMAX ) ).GE.ALPHA*ROWMAX */
		...
		if (! ((r__1 = a[imax + imax * a_dim1], dabs(r__1)) < alpha * 
			rowmax)) {
...
/*              the interchanges in columns k+1:N. */
		...
		if (k < *n) {
		    i__1 = *n - k;
		    sswap_(&i__1, &a[k + (k + 1) * a_dim1], lda, &a[p + (k + 
			    1) * a_dim1], lda);
		}
...
/*                       = A - W(k)*1/D(k)*W(k)**T */
			...
			i__1 = k - 1;
			r__1 = -d11;
			ssyr_(uplo, &i__1, &r__1, &a[k * a_dim1 + 1], &c__1, &
...
/*                       = A - W(k)*(1/D(k))*W(k)**T */
			...
			i__1 = *n - k;
			r__1 = -d11;
			ssyr_(uplo, &i__1, &r__1, &a[k + 1 + k * a_dim1], &
