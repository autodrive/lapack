	...
	real *delta, real *rho, real *sigma, real *work, integer *info)
{
    ...
    static real dw, zz[3];
    static integer ip1;
    static real sq2, eta, phi, eps, tau, psi;
    static integer iim1, iip1;
    static real tau2, dphi, sglb, dpsi, sgub;
    static integer iter;
    static real temp, prew, temp1, temp2, dtiim, delsq, dtiip;
...
/*           We choose d(i) as origin. */
	    ...
	    if (a > 0.f) {
		tau2 = b * 2.f / (a + sqrt((r__1 = a * a - b * 4.f * c__, 
			dabs(r__1))));
	    } else {
		tau2 = (a - sqrt((r__1 = a * a - b * 4.f * c__, dabs(r__1)))) 
			/ (c__ * 2.f);
	    }
...
/*           SIGMA - D( I ). */
	    ...
	    temp = sqrt(eps);
	    if (d__[*i__] <= temp * d__[ip1] && (r__1 = z__[*i__], dabs(r__1))
		     <= temp && d__[*i__] > 0.f) {
/* Computing MIN */
		r__1 = d__[*i__] * 10.f;
		tau = dmin(r__1,sgub);
...
/* Computing 2nd power */
			...
			eta = (a - sqrt((r__1 = a * a - b * 4.f * c__, dabs(
				r__1)))) / (c__ * 2.f);
		    } else {
