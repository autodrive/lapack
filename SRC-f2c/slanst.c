...
/*        Find norm1(A). */
	...
	if (*n == 1) {
	    ...
	    anorm = dabs(d__[1]) + dabs(e[1]);
	    sum = (r__1 = e[*n - 1], dabs(r__1)) + (r__2 = d__[*n], dabs(r__2)
		    );
	    if (anorm < sum || sisnan_(&sum)) {
		anorm = sum;
	    }
		...
		sum = (r__1 = d__[i__], dabs(r__1)) + (r__2 = e[i__], dabs(
			r__2)) + (r__3 = e[i__ - 1], dabs(r__3));
		if (anorm < sum || sisnan_(&sum)) {
		    anorm = sum;
		}
/* L20: */
