...
/*  ===================================================================== */
doublereal clanht_(char *norm, integer *n, real *d__, complex *e, ftnlen 
	norm_len)
{
...
/*        Find norm1(A). */
	...
	if (*n == 1) {
	    ...
	    anorm = dabs(d__[1]) + c_abs(&e[1]);
	    sum = c_abs(&e[*n - 1]) + (r__1 = d__[*n], dabs(r__1));
	    if (anorm < sum || sisnan_(&sum)) {
		anorm = sum;
	    }
	    i__1 = *n - 1;
	    for (i__ = 2; i__ <= i__1; ++i__) {
		sum = (r__1 = d__[i__], dabs(r__1)) + c_abs(&e[i__]) + c_abs(&
			e[i__ - 1]);
		if (anorm < sum || sisnan_(&sum)) {
		    anorm = sum;
		}
