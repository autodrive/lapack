/* csysv_aa.f -- translated by f2c (version 20100827).
...
/*  ===================================================================== */
/* Subroutine */ int csysv_aa__(char *uplo, integer *n, integer *nrhs, 
	...
	complex *work, integer *lwork, integer *info, ftnlen uplo_len)
{
    ...
    /* Local variables */
    extern /* Subroutine */ int csytrf_aa__(char *, integer *, complex *, 
	    integer *, integer *, complex *, integer *, integer *, ftnlen), 
	    csytrs_aa__(char *, integer *, integer *, complex *, integer *, 
...
/* Computing MAX */
    ...
    if (*info == 0) {
	csytrf_aa__(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], &c_n1, 
		info, (ftnlen)1);
	lwkopt_sytrf__ = (integer) work[1].r;
	csytrs_aa__(uplo, n, nrhs, &a[a_offset], lda, &ipiv[1], &b[b_offset], 
...
/*     Compute the factorization A = U*T*U**T or A = L*T*L**T. */
    ...
    csytrf_aa__(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], lwork, info, (
	    ftnlen)1);
    if (*info == 0) {
