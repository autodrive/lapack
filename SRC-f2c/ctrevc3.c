...
/* Subroutine */ int ctrevc3_(char *side, char *howmny, logical *select, 
	...
	integer *lwork, real *rwork, integer *lrwork, integer *info, ftnlen 
	side_len, ftnlen howmny_len)
{
    ...
    extern /* Subroutine */ int csscal_(integer *, real *, complex *, integer 
	    *), claset_(char *, integer *, integer *, complex *, complex *, 
	    complex *, integer *, ftnlen), clacpy_(char *, integer *, integer 
	    *, complex *, integer *, complex *, integer *, ftnlen), xerbla_(
	    char *, integer *, ftnlen);
...
/*                 normalize vectors */
			...
			i__3 = ii + (nb + k) * *n;
			remax = 1.f / ((r__1 = work[i__3].r, dabs(r__1)) + (
				r__2 = r_imag(&work[ii + (nb + k) * *n]), 
				dabs(r__2)));
...
/* Computing MAX */
	    i__3 = ki + ki * t_dim1;
	    r__3 = ulp * ((r__1 = t[i__3].r, dabs(r__1)) + (r__2 = r_imag(&t[
		    ki + ki * t_dim1]), dabs(r__2)));
	    smin = dmax(r__3,smlnum);
...
/*           [ T(KI+1:N,KI+1:N) - T(KI,KI) ]**H * X = SCALE*WORK. */
		...
		i__4 = k + k * t_dim1;
		if ((r__1 = t[i__4].r, dabs(r__1)) + (r__2 = r_imag(&t[k + k *
			 t_dim1]), dabs(r__2)) < smin) {
		    i__5 = k + k * t_dim1;
...
/*              no back-transform: copy x to VL and normalize. */
		...
		i__3 = ii + is * vl_dim1;
		remax = 1.f / ((r__1 = vl[i__3].r, dabs(r__1)) + (r__2 = 
...
/*                 normalize vectors */
			...
			ii = icamax_(n, &work[(nb + k) * *n + 1], &c__1);
			i__4 = ii + (nb + k) * *n;
			remax = 1.f / ((r__1 = work[i__4].r, dabs(r__1)) + (
				r__2 = r_imag(&work[ii + (nb + k) * *n]), 
				dabs(r__2)));
			csscal_(n, &remax, &work[(nb + k) * *n + 1], &c__1);
		    }
