...
/*  ===================================================================== */
/* Subroutine */ int csytf2_rk__(char *uplo, integer *n, complex *a, integer *
	lda, complex *e, integer *ipiv, integer *info, ftnlen uplo_len)
{
    ...
    static complex t, d11, d12, d21, d22;
...
/*                 Determine both ROWMAX and JMAX. */
		...
		if (imax > 1) {
		    ...
		    stemp = (r__1 = a[i__1].r, dabs(r__1)) + (r__2 = r_imag(&
			    a[itemp + imax * a_dim1]), dabs(r__2));
		    if (stemp > rowmax) {
			rowmax = stemp;
			jmax = itemp;
		    }
...
/*                 ABS( A( IMAX, IMAX ) ).GE.ALPHA*ROWMAX */
		...
		if (! ((r__1 = a[i__1].r, dabs(r__1)) + (r__2 = r_imag(&a[
			imax + imax * a_dim1]), dabs(r__2)) < alpha * rowmax))
			 {
